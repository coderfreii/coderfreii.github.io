...
​		我们配置了mybatis的xml文件的路径，mybatis会根据这个路径解析这些xml文件，一边随后跟具体的Mapper的代理对象通过资源表示绑定，绑定规则是根据  xml里面的命名空间，每个标签对应的id 等等。

#### 		代理类的生成

​		mybatis的代理类生成逻辑和其它大部分的代理增强类的逻辑不同，一般而言spring中大部分情况的增强逻辑会放在bean的后置处理器中完成，有专门的后置处理器来做这件事情，但mybatis的做法不同，它的的代理类是通过自定义beandefinition 实现。

​		简单地将就是， mybatis 自定义了 关于mapper接口的beandefinition 的扫描和注册，并修改了beandefinition 中beanclass 为一个工厂类，导致通过该bean的名字实例化时会生成该工厂类的实例，所有的增强工作会在该工厂中完成。

​		为什么mybatis 要这么做呢？

​		因为这样mybatis 可以比较自由地根据自己地上下文去做映射，更精细地去控制代理类的代理行为。

#### 	关于扩展点

​		这里就不赘述了，网上一搜一大把，我也没有去记忆这些东西，因为没必要，如果想自己寻找扩展点，在代理类调用时断点即可。

什么一级缓存，二级缓存，各种拦截器，所有这些逻辑都在这随后执行。
